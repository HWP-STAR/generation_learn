...
# VAE loss curve
if os.path.exists("results/exp1_cifar_vae_loss.txt"):
    epochs, vae_losses = read_vae_loss("results/exp1_cifar_vae_loss.txt")
    plt.figure(figsize=(10, 5))
    plt.plot(epochs, vae_losses)
    plt.xlabel("Epoch")
    plt.ylabel("VAE Loss")
    plt.title("Phase 1: CNN VAE Unsupervised Training Loss (CIFAR-100)")
    plt.grid(True)
    plt.savefig("results/cifar_vae_loss_curve.png")
    print("Saved results/cifar_vae_loss_curve.png")

# Classification comparison
exp_names = {
    "results/exp1_cifar_results.txt": "Exp1: Frozen Encoder",
    "results/exp2_cifar_results.txt": "Exp2: Fine-tune Encoder",
    "results/exp3_cifar_results.txt": "Exp3: Standalone Classifier",
}
...
if files:
    ...
    axes[0].set_title("CIFAR-100: Training Loss Comparison")
    axes[0].legend()
    axes[0].grid(True)

    axes[1].set_xlabel("Epoch")
    axes[1].set_ylabel("Train Accuracy (%)")
    axes[1].set_title("CIFAR-100: Training Accuracy Comparison")
    axes[1].legend()
    axes[1].grid(True)

    axes[2].set_xlabel("Epoch")
    axes[2].set_ylabel("Test Accuracy (%)")
    axes[2].set_title("CIFAR-100: Test Accuracy Comparison")
    axes[2].legend()
    axes[2].grid(True)

    plt.tight_layout()
    plt.savefig("results/cifar_comparison.png")
    print("Saved results/cifar_comparison.png")
